<div class="w-full md:px-24 px-5 py-24 rounded-3xl relative">
    <div style="background: url('{{ asset('images/image 7.webp') }}'); background-size: cover; background-position: center;"
        class="w-full h-[359px] relative rounded-3xl flex flex-col items-center justify-center">

        <div class="bg-[#141D38E5] w-full h-full absolute top-0 left-0 z-10 rounded-3xl"></div>

        <h1
            class="text-white font-extrabold md:leading-[60px] leading-10 md:text-[40px] z-20 relative text-center px-3 md:w-[857px] mb-5 text-2xl md:mb-12">
            Ready to optimize
            your HR processes and boost your workforce efficiency?</h1>
        <a href="/contact" class="bg-primary w-fit text-white z-20 px-8 py-3 rounded-full md:text-lg font-medium">
            Consult us today
        </a>
    </div>
</div>

<footer
    class="w-full md:py-40 py-16 relative bg-[#141D38] flex md:flex-row flex-col 2xl:gap-36 md:gap-32 gap-8 md:px-24 px-5">
    <div class="flex flex-col gap-6 flex-grow min-w-[200px]">
        <img src="{{ asset('images/mel-hr.svg') }}" class="w-36 z-10" alt="">
        <p class="text-white z-10 ">MEL HR Services is a leading HR consulting firm in Nigeria, offering expert labor
            law advisory, payroll management, recruitment, and employee management solutions to help businesses
            thrive.</p>
    </div>

    <div class="flex flex-col gap-2 flex-grow z-10 max-w-[200px]">
        <h6 class="text-[#B2B5BD] z-10 text-xl font-semibold max-w-[153px]">Link</h6>
        <ul class="flex flex-col gap-2 z-10">
            <li><a href="/" class="text-white z-10 min-w-[153px]">Home</a></li>
            <li><a href="/" class="text-white z-10">About us</a></li>
            <li><a href="/" class="text-white z-10">Service</a></li>
            <li><a href="/" class="text-white z-10">Contact</a></li>
        </ul>
    </div>

    <div class="flex flex-col gap-2 flex-grow z-10 min-w-[200px]">
        <h6 class="text-[#B2B5BD] z-10 text-xl font-semibold">Address</h6>
        <p class="text-white min-w-[153px]">Suite 207, AHCN Towers, CIPM Avenue,
            Alausa
            CBD,
            Ikeja, Lagos.</p>
    </div>

    <div class="flex flex-col gap-2 flex-grow z-10 min-w-[200px]">
        <h6 class="text-[#B2B5BD] z-10 text-xl">Contact us</h6>
        <p class="text-white font-semibold min-w-[250px]">Email:</p>
        <p class="text-white min-w-[250px]">hrs@mel-ng.com</p>
        <p class="text-white min-w-[250px]">contact@mel-ng.com</p>
    </div>
    <div class="absolute top-0 left-0 w-full h-full">
        <img src="{{ asset('images/Footer background.png') }}" class="w-full h-full z-0" alt="">
    </div>
</footer>

<div class="md:px-24 px-5 w-full bg-primary py-5 flex md:flex-row flex-col gap-3 justify-between">
    <p class="text-white">Copyright © 2024 MEL HR Service. All Rights Reserved.</p>
    <p class="text-white">Terms & Condition | Careers | Privacy policy</p>
</div>
